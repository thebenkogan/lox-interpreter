...
func (e *ForStatement) String() string {
	initStr := ""
	if e.Init != nil {
		initStr = e.Init.String()
	}
	conditionStr := ""
	if e.Condition != nil {
		conditionStr = e.Condition.String()
	}
	incrementStr := ""
	if e.Increment != nil {
		incrementStr = e.Increment.String()
	}
	return fmt.Sprintf("for (%s; %s; %s) %s", initStr, conditionStr, incrementStr, e.Body.String())
}

func (e *ForStatement) Execute(env *Environment, output io.Writer) *RuntimeError {
	env.CreateScope() // so initializer declaration is removed after for loop
	defer env.ExitScope()
	if e.Init != nil {
		if err := e.Init.Execute(env, output); err != nil {
			return err
		}
	}
	for {
		...
		if e.Increment != nil {
			if _, err := e.Increment.Evaluate(env); err != nil {
				return err
			}
		}
	}
